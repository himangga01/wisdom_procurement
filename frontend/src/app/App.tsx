...
type NavGroupTone = "overview" | "notice" | "document" | "rag" | "admin" | "settings";
...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/judgment-runs", icon: ClipboardCheck, label: "판단 검토", note: "부족조건, 근거 링크, 검토 상태 관리" },
...
const pageMeta = [
  ...
  },
  {
    ...
    description: "상태 요약, 실패 목록, 검토 대기 작업을 확인합니다.",
  },
  {
    match: (pathname: string) => pathname.startsWith("/corporations"),
    eyebrow: "법인 관리",
    title: "법인 기본 정보 관리",
    description: "공고 검토에 필요한 법인 프로필과 증빙자료를 안정적으로 관리합니다.",
  },
  {
  ...
  },
  {
    ...
    eyebrow: "규칙 후보",
    title: "기준 규칙 후보 검토",
    description: "기준문서에서 추출한 조건 후보를 승인, 반려, 수정하며 근거 연결 품질을 관리합니다.",
  },
  {
  ...
  },
  {
    ...
    eyebrow: "판단 검토",
    title: "부족조건 판단 검토",
    description: "공고 요구조건, 법인 준비상태, 기준문서 근거를 묶어 검토 가능한 결과로 정리합니다.",
  },
  {
  ...
  },
  {
    ...
    eyebrow: "검색 평가",
    title: "기준문서 검색 품질 확인",
    description: "평가 질의셋별 검색 품질과 누락 근거를 확인합니다.",
  },
];
...
function navDemoId(to?: string) {
  ...
  return to ? demoIds[to] : undefined;
}
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <nav className="nav-stack" aria-label="Primary Navigation">
          {navGroups.map((group) => (
            <div className={`nav-section nav-section--${group.tone}`} key={group.title}>
                ...
                item.to ? (
                  <div key={item.label} className="nav-card-row">
                    <NavLink
                      ...
                      data-demo-id={navDemoId(item.to)}
