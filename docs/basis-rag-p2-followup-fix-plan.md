...
- 이 문서의 P2 후속 이슈 3건은 구현 완료 상태입니다.
- 검색 평가 결과 source는 `json_basis_index`로 기록됩니다.
- 기준문서 삭제는 JSON 인덱스가 missing/corrupt/inconsistent이면 409로 차단합니다.
- 기준문서 검색, 규칙 후보 승인, Phase 3 판단 citation은 동일하게 JSON basis index 건강 상태를 요구합니다.
- 최근 PDF/RAG 보강 후 전체 backend 기준선은 `134 passed`, `8 skipped`입니다.
...
- The three P2 follow-up issues in this document are implemented.
- Retrieval evaluation stores `json_basis_index` as the source.
- Basis-document deletion returns 409 when the JSON index is missing, corrupt, or inconsistent.
- Basis search, rule-candidate approval, and Phase 3 judgment citations all require JSON basis-index health.
...
Target issues:
- retrieval evaluation result metadata used to point at the old DB-chunk source wording
