...
- 향후 부족조건/준비 상태 판단을 위해 법인 상세 화면은 `준비 상태 프로필` 형태로 확장한다.
...
- 권장 탭
  ...
  - 증빙자료
  - 판단 이력
- 각 탭에는 `이 정보가 공고 준비 상태 판단에 왜 필요한지`를 짧게 설명한다.
...
- 법인 프로필 준비도 카드는 최종 자격 판정이 아니라 부족한 법인 정보와 증빙을 빠르게 알려주는 안내 UI다.
...
- 최종 자격 판단처럼 보이는 표현은 Phase 1.5에서 사용하지 않는다.
...
- 상단: 준비 상태 배지
  ...
  - 확인 필요
  - 부족 조건
- 결과 화면의 핵심은 확정 판정을 강조하는 것이 아니라 `현재 부족한 조건`과 `준비해야 할 것`을 빠르게 파악하게 하는 것이다.
