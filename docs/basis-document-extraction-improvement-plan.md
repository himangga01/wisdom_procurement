...
이 문서는 TXT/DOCX/MD 기준 파일과 서비스 추출 결과를 비교해 추출 로직을 보완하기 위한 계획/분석 문서입니다.
문서 안의 초기 비교 수치 중 일부는 PyMuPDF 기반 기준선입니다. 현재 서비스 기본 PDF reader는 OpenDataLoader 우선 `auto` 모드로 변경되었습니다.

현재 구현 상태:
- OpenDataLoader PDF가 Markdown/JSON/table metadata를 우선 추출합니다.
- PyMuPDF는 fallback과 OCR 렌더링 보조 엔진으로 유지합니다.
- 기준문서 table metadata는 `table_row` chunk와 JSON basis index payload에 반영됩니다.
- DOCX 기준 비교와 실제 DOCX 업로드 분석은 문단뿐 아니라 표 cell 텍스트도 포함합니다.
- JSON basis index가 손상/누락/불일치 상태이면 RAG 검색과 citation 사용을 차단합니다.
- 최근 PDF/RAG 회귀 테스트 기준선은 전체 backend `134 passed`, `8 skipped`입니다.

남은 해석 기준:
- PyMuPDF Markdown 재생성 수치는 역사적 baseline입니다.
- 현재 기준문서 RAG 품질 판단은 OpenDataLoader QA 산출물과 table-row token coverage를 우선 봅니다.
- exact string 기반 table-row coverage는 기준 MD와 OpenDataLoader의 셀 병합/줄바꿈 차이 때문에 보조 지표로만 사용합니다.
...
This document analyzes service extraction against TXT/DOCX/MD references. Some early metrics are PyMuPDF baselines. The current default PDF reader is now OpenDataLoader-first `auto` mode.

Current implementation:
- OpenDataLoader PDF extracts Markdown/JSON/table metadata first.
- PyMuPDF remains fallback and OCR rendering helper.
- Basis table metadata feeds `table_row` chunks and JSON basis-index payloads.
- DOCX comparison and production DOCX parsing include table-cell text.
- Invalid/missing/inconsistent JSON basis index state blocks RAG retrieval and citation usage.
- Latest PDF/RAG backend baseline: `134 passed`, `8 skipped`.

Interpretation:
- PyMuPDF Markdown regeneration metrics are historical baselines.
- Current basis RAG quality should prioritize OpenDataLoader QA artifacts and table-row token coverage.
- Exact string-based table-row coverage is secondary because the reference MD and OpenDataLoader differ in cell merging, line breaks, and whitespace.
