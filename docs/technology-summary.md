...
## 한줄 요약
`SMART 조달청 계산기`는 로컬 PC에서 실행되는 조달문서/나라장터 공고 분석 포탈이며, 핵심은 `공고/문서 수집 -> OpenDataLoader/PyMuPDF/DOCX 표 포함 텍스트 추출 -> OCR 후보 판정 -> AI 구조화 요약 -> JSON 기준문서 인덱스 기반 RAG -> 부족조건 중심 판단/운영 관리`입니다.

## 현재 코드 기준 업데이트
최종 갱신일: 2026-06-07

- PDF 기본 추출 엔진은 OpenDataLoader 우선 `auto` 모드입니다.
- PyMuPDF는 fallback과 OCR 렌더링 보조 엔진입니다.
- DOCX는 `python-docx`로 문단과 표 cell 텍스트를 함께 추출합니다.
- 기준문서 검색 source는 `basis_document_chunks` 직접 검색이 아니라 `storage/basis-index/basis-index.json`입니다.
- 판단 엔진은 최종 판정 대신 부족 조건, 필요 서류, 준비 가이드, citation 상태를 저장합니다.
- 운영 기능은 대시보드, 작업 이력/실패, 백업/검증/복원계획 dry-run을 포함합니다.
...
핵심 방식:
- PDF 기본 추출 엔진은 `OpenDataLoader PDF` 우선 `auto` 모드입니다.
- OpenDataLoader가 Markdown/JSON/table metadata, page number, bbox를 추출합니다.
- Java 미설치, timeout, 변환 실패 시 `PyMuPDF` fallback으로 페이지별 텍스트, 블록 수, 문자 수, OCR 필요 여부를 남깁니다.
...
| 기술 | 용도 |
|---|---|
| OpenDataLoader PDF | PDF Markdown/JSON/table metadata/page/bbox 추출 기본 엔진 |
| PyMuPDF | PDF fallback, 페이지 텍스트/블록/렌더링/OCR 보조 |
| python-docx | DOCX 문단/표 cell 텍스트 추출 |
...
| 단계 | 핵심 기술 |
|---|---|
| Phase 1 | React 포탈, 로컬 DB, 파일 업로드, OpenDataLoader/PyMuPDF/DOCX 파싱, AI 요약, 재분석 |
| Phase 1.5 | 나라장터 API, 공고 검색, 저장한 공고, 첨부 PDF/DOCX 다운로드, 공고 분석 |
| Phase 2 | 기준 PDF 관리, OCR degrade, 자동 청킹, JSON basis index, 규칙 후보 관리 |
| Phase 3 | 법인-공고 요건 매칭, RAG 근거 검색, 부족조건 판단 엔진, 체크리스트/가이드 생성 |
| Phase 4 | 운영 대시보드, 작업/실패 관리, 백업/검증/복원계획 |

## 현재 주의할 점
- HWP/HWPX는 현재 범위 제외입니다.
- Phase 1.5 단독 범위는 요약까지였고, 현재 판단 기능은 Phase 3 `judgment_runs` 범위에서 부족조건 중심으로 동작합니다.
- 나라장터 API 키는 전체 값을 화면, 로그, 문서에 노출하면 안 됩니다.
- 설계상 백엔드는 FastAPI 권장이지만 현재 구현은 Flask이므로 다음 구현 전에 방향을 결정해야 합니다.
- 기준문서 RAG와 판단 엔진은 구현되어 있지만, citation이 없거나 약한 조건은 확정 근거처럼 사용하지 않습니다.
...
## One-Line Summary
The service is a local-first procurement document and Nara Marketplace notice analysis portal. Its core pipeline is `ingest notice/document -> extract PDF/DOCX text with OpenDataLoader/PyMuPDF/DOCX table support -> detect OCR need -> structured AI summary -> JSON basis-index RAG -> gap-first judgment and operations management`.

## Current Code Update
Last updated: 2026-06-07

- PDF extraction defaults to OpenDataLoader-first `auto` mode.
- PyMuPDF remains fallback and OCR rendering helper.
- DOCX extraction includes paragraphs and table cells.
- Basis retrieval uses `storage/basis-index/basis-index.json`.
- The judgment engine stores missing requirements, required documents, preparation guidance, and citation status.
- Phase 4 operations cover dashboard, operation runs/failures, and backup validation/restore dry-runs.
...
- PDF extraction: OpenDataLoader PDF in `auto` mode by default.
- OpenDataLoader extracts Markdown/JSON/table metadata, page numbers, and bounding boxes.
- Java/package/timeout/conversion failures fall back to `PyMuPDF`.
...
- OpenDataLoader PDF
- PyMuPDF fallback
- python-docx paragraph/table-cell extraction
- PaddleOCR
- Tesseract
...
|---|---|
| Phase 1 | React portal, SQLite, file upload, OpenDataLoader/PyMuPDF/DOCX parsing, AI summary, re-analysis |
| Phase 1.5 | Nara API, notice board, Saved Notices, PDF/DOCX attachment download, notice analysis |
| Phase 2 | Basis PDF ingestion, OCR degrade, automatic chunking, JSON basis index, rule-candidate review |
| Phase 3 | corporation-notice matching, RAG retrieval, gap-first judgment engine, checklist/guide generation |
| Phase 4 | operations dashboard, operation/failure management, backup validation and restore dry-runs |
