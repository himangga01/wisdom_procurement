...
핵심 방식:
- 주 모델은 `OpenAI GPT-5.1`입니다.
- 보조/저비용 모델은 `GPT-5 mini`입니다.
...
| TanStack Query | API 조회/캐싱/상태 관리 |
| React Hook Form | 입력 폼 관리 권장 |
| Node.js 20+ | 프론트엔드 개발/빌드 런타임 |

### 백엔드
| 기술 | 용도 |
|---|---|
| Python 3.12+ | 문서 처리, API 서버, AI 연동 |
...
주의:
- 설계 문서는 FastAPI 기준으로 작성되어 있습니다.
- 현재 작업 로그 기준 실제 구현은 Flask 단일 파일 백엔드가 존재합니다.
- 이후 구현 단계에서 Flask 유지 또는 FastAPI 전환 결정을 명확히 해야 합니다.
...
| 기술 | 용도 |
|---|---|
| OpenAI GPT-5.1 | 주 요약 모델 |
| GPT-5 mini | 저비용 보조 모델 |
...
### Backend
- Python 3.12+
- FastAPI is the recommended architecture target.
- Current Phase 1 runtime uses Flask according to work-log/project state.
- SQLAlchemy and Pydantic are recommended for ORM/schema layers.
