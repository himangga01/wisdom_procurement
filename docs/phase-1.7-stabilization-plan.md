...
Phase 1.7은 최종 자격 판단 엔진이 아닙니다. 목표는 저장된 나라장터 공고의 요구조건 후보와 사용자가 관리하는 법인 프로필/증빙자료를 비교해, 현재 법인이 무엇을 준비했고 무엇이 부족할 가능성이 있는지 빠르게 확인하도록 돕는 것입니다.
...
## 진행 상태
- `2026-05-22`: 나라장터 API에서 실제 공고 PDF 5개를 내려받아 `backend/tests/nara-pdf-samples/`에 로컬 QA 샘플로 저장했다.
- `2026-05-22`: 샘플 PDF는 Git에 커밋하지 않도록 `.gitignore`에 제외 경로를 추가했다.
- `2026-05-22`: `scripts/fetch-nara-phase17-samples.py`를 추가해 나라장터 공고 검색, PDF 다운로드, 파싱, 요구조건 후보 추출, 최종 판정 문구 금지 검사를 한 번에 수행하도록 했다.
- `2026-05-22`: `backend/tests/test_nara_phase17_live_samples.py`를 추가해 `RUN_NARA_PHASE17_QA=1`일 때만 실제 샘플 PDF 기반 QA 테스트를 실행하도록 했다.
- `2026-05-22`: 고유 공고 5건의 PDF 샘플에서 텍스트 추출과 요구조건 후보 추출 QA가 통과했다.
...
완료 기준:
- 사용자가 화면만 보고 “이것은 최종 자격 판단이 아니라 부족조건 미리보기”임을 이해할 수 있다.
...
## Progress
- `2026-05-22`: downloaded five real Nara notice PDFs through the public API into `backend/tests/nara-pdf-samples/` as local QA samples.
- `2026-05-22`: added the sample directory to `.gitignore` so real downloaded PDFs are not committed.
- `2026-05-22`: added `scripts/fetch-nara-phase17-samples.py` to search notices, download PDFs, parse text, extract requirement candidates, and check that no final-verdict wording is emitted.
- `2026-05-22`: added `backend/tests/test_nara_phase17_live_samples.py`, which runs only when `RUN_NARA_PHASE17_QA=1`.
- `2026-05-22`: Phase 1.7 QA passed on five unique real notice PDF samples.
