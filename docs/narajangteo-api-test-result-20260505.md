...
|---|---:|
| 전체 첨부 | 39 |
| 처리 가능 첨부(PDF/DOCX) | 14 |
| 처리 제외 첨부(HWP/HWPX/XLSX 등) | 25 |
...
- 현재 `scripts/test-nara-api.py`는 첫 번째 처리 가능 PDF/DOCX 첨부를 자동으로 내려받아 HTTP 상태, Content-Type, 다운로드 바이트, 파일 시그니처를 결과 JSON에 기록합니다.
...
-> PyMuPDF/python-docx 텍스트 추출
   -> 선택한 AI 모델(`gemini-2.5-flash` 기본값) 요약 또는 fallback 요약
-> 분석 결과 저장
```
